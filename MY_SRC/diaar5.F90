...
!!AW
      REAL(wp), ALLOCATABLE, DIMENSION(:,:)     :: zbotpres_b       ! 2D workspace 
      REAL(wp), ALLOCATABLE, DIMENSION(:,:,:) ::   zrhd_b, zrhop_b ! used for my derivative hack
      REAL(wp), ALLOCATABLE, DIMENSION(:,:,:,:) :: zts_b                       ! 4D workspace
!!AW end
      ...
      IF( l_ar5 ) THEN 
         ...
         ALLOCATE( zrhd_b(jpi,jpj,jpk), zrhop_b(jpi,jpj,jpk) )
         ALLOCATE( zts_b(jpi,jpj,jpk,jpts) )
         ALLOCATE( zbotpres_b(jpi,jpj) )
         zarea_ssh(:,:) = area(:,:) * sshn(:,:)
      ENDIF
      !
...
!!AW making some changes to output time derivative of bottom pressure
      IF( iom_use( 'botpres' ) .OR. iom_use( 'sshthster' )  .OR. iom_use( 'sshsteric' ) .OR. iom_use( 'botpres_dt' ) .OR. iom_use( 'rho_in' ) ) THEN    
!!AW end
         !                     
...
!!AW
         zts_b(:,:,:,:) = tsb(:,:,:,:)
         CALL eos( zts_b, zrhd_b, zrhop_b, gdept_b(:,:,:) )                 ! now in situ and potential density
         zrhop_b(:,:,jpk) = 0._wp
         zbotpres_b(:,:) = 0._wp
         DO jk = 1, jpkm1
            zbotpres_b(:,:) = zbotpres_b(:,:) + e3t_b(:,:,jk) * zrhd_b(:,:,jk) * tmask(:,:,jk)
         END DO
         zbotpres_b(:,:) = zztmp * ( zbotpres_b(:,:) + sshb(:,:) + thick0(:,:) )
         CALL iom_put( 'botpres_dt', ( ( zbotpres - zbotpres_b ) / rdt )*tmask(:,:,1) )
!!AW end

!!AW output insitu density
         zrhd(:,:,jpk) = 0._wp
         CALL iom_put( 'rho_in', zrhd )
!!AW end
