...
!!AW add some temp variables to store max velocity in zonal and merid
      REAL(wp)               ::   zumax, zvmax
!!AW end
      !!----------------------------------------------------------------------
      !
      ...
      ENDIF
!!AW make it so max velocity uses both zonal and meridional compoenents rather than just zonal
      zumax = MAXVAL(  ABS( un(:,:,:) )  )                                  ! velocity max (zonal)
      zvmax = MAXVAL(  ABS( vn(:,:,:) )  )                                  ! velocity max (merdional)
      zmax(2) = MAX(  zumax, zvmax  )                                    ! velocity max
!!AW end      
