...
CONTAINS
   ...
   SUBROUTINE dyn_ldf( kt )
      ...
      ! DB: modfications to rotate the laplacian operator
      !     while using along-level bilaplacian diffusion
      CASE ( 5 )                                        ! rotated laplacian + iso-level bilaplacian (s-coord)
         CALL dyn_ldf_iso    ( kt )
         CALL dyn_ldf_bilap  ( kt )
      ! DB
      !
   ...
   SUBROUTINE dyn_ldf_init
      ...
      IF(lwp) THEN
         ...
         IF( nldf ==  5 )   WRITE(numout,*) '              rotated laplacian and iso-level bilaplacian operators'
      ENDIF
      !
